...
pub async fn login_handler(
    ...
    let user = sqlx::query("SELECT password_hash FROM reacher_users WHERE email = $1")
        ...
        .ok_or_else(|| ReacherResponseError::new(StatusCode::UNAUTHORIZED, "Invalid email or password"))?;

    let password_hash: String = sqlx::Row::get(&user, "password_hash");

    // Check password. For now, supporting plain text for the initial migration, but we should hash it.
    let is_valid = if password_hash.starts_with("$2") {
        verify(&body.password, &password_hash).unwrap_or(false)
    } else {
